...
| `docker-compose.hot.yml` | Blue/green Docker Compose configuration for hot deployment |
...
### Blue/Green Hot Deployment

Use this when you want the public port to stay online while deploying a new Docker image. The stack runs nginx on `SERVER_PORT` and switches traffic between `sub2api-blue` and `sub2api-green` only after the new slot passes `/health`.

First-time hot deploy from a cloned repository:

```bash
cd deploy
cp .env.example .env
nano .env  # Set POSTGRES_PASSWORD, JWT_SECRET, TOTP_ENCRYPTION_KEY, etc.
mkdir -p data postgres_data redis_data
bash hot-deploy.sh --build
```

Deploy a published image instead of building locally:
...
Update from GitHub with a local Docker build:
...
Do not build on very small production servers. `--build` compiles the frontend and Go backend and can put enough CPU/memory pressure on a 1-2GB host to affect live traffic. The script refuses `--build` below `HOT_DEPLOY_MIN_BUILD_MEM_MB` unless `ALLOW_LOW_RESOURCE_BUILD=true` is set.

Recommended low-resource flow:

```bash
# Build on a workstation or CI runner.
IMAGE_TAG=sub2api-hot:0.1.129 bash deploy/build_image.sh --save /tmp/sub2api-hot-0.1.129.tar.gz

# Copy the archive to the server, then deploy from the runtime directory.
scp /tmp/sub2api-hot-0.1.129.tar.gz root@server:/tmp/
cd /data/sub2api
KEEP_OLD=true DRAIN_SECONDS=300 bash hot-deploy.sh --load-image /tmp/sub2api-hot-0.1.129.tar.gz --keep-old
```

If you are migrating from `docker-compose.local.yml`, the script can take over from the legacy `sub2api` container. It starts the new blue/green slot first, then stops only the old app container so nginx can bind the public port while PostgreSQL and Redis keep running:
...
```bash
# Keep the old slot running after traffic switches, useful for very long streams.
KEEP_OLD=true bash hot-deploy.sh --build

# Increase the default 15 second drain window before the old slot is stopped.
DRAIN_SECONDS=120 bash hot-deploy.sh --build

# Skip image pulling when deploying a local image tag.
bash hot-deploy.sh --image sub2api:local --no-pull

# Load a docker save archive before switching traffic.
bash hot-deploy.sh --load-image /tmp/sub2api-hot.tar.gz --keep-old
```

Notes:

- Database migrations must be backward compatible with the currently running version. The app serializes migrations with a PostgreSQL advisory lock, but no deployment script can make a breaking schema change zero-downtime.
- Daily hot deploys pull and replace only the Sub2API app image; PostgreSQL and Redis are not recreated as part of an app release.
- If both `./config.yaml` and `./data/config.yaml` exist, they must match. Older compose files could bind `./config.yaml` directly, while the hot deploy compose mounts only `./data`.
- The old slot is stopped after the switch by default to avoid duplicate background jobs. Use `KEEP_OLD=true` only when you plan to stop it manually after active streams drain.
- Runtime files generated by the script live under `deploy/nginx/conf.d/` and `deploy/.hotdeploy/`.

### Manual GitHub Actions Deployment

The repository includes a manual workflow at `.github/workflows/deploy.yml`. It appears in GitHub under **Actions -> Deploy to server -> Run workflow**.

Before running it, configure these repository secrets in **Settings -> Secrets and variables -> Actions**:

| Secret | Required | Example | Notes |
|--------|----------|---------|-------|
| `SERVER_HOST` | Yes | `47.85.29.80` | Server IP or DNS name |
| `SERVER_USER` | Yes | `root` | SSH user with Docker access and write access to `deploy_path` |
| `SERVER_SSH_KEY` | Yes | private key text | Private key used by GitHub Actions to SSH into the server |
| `SERVER_PORT` | No | `22` | SSH port; defaults to `22` |
| `SERVER_SSH_KNOWN_HOSTS` | Recommended | output of `ssh-keyscan -p 22 47.85.29.80` | Pins the server host key; if omitted, the workflow scans it during the run |
