...
watch(() => props.user, (u) => {
  if (u) {
    Object.assign(form, { email: u.email, password: '', username: u.username || '', role: u.role || 'user', notes: u.notes || '', concurrency: u.concurrency, rpm_limit: u.rpm_limit ?? 0, customAttributes: {} })
...
const handleUpdateUser = async () => {
  ...
  try {
    const data: any = { email: form.email, username: form.username, role: form.role, notes: form.notes, concurrency: form.concurrency, rpm_limit: form.rpm_limit }
