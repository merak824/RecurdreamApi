...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		openAITTFTContext         sql.NullString
	)
	...
	if openAITTFTContext.Valid && strings.TrimSpace(openAITTFTContext.String) != "" {
		var value service.OpenAITTFTContext
		if err := json.Unmarshal([]byte(openAITTFTContext.String), &value); err == nil {
			log.OpenAITTFTContext = &value
		}
	}
